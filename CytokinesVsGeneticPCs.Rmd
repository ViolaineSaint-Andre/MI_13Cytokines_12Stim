...
# Access to individuals’ genetic data (LabExMI_ancestry_PCs.csv) is provided for research use only after review and approval by the Milieu Intérieur data access committee, in line with patient privacy and confidentiality agreements. Requests can be sent to milieuinterieurdac@pasteur.fr.

df_data_all <- read.table(file = paste0("./DM_2018-09-04_13-49-19.csv"),
                          dec = ",",
                          sep=';', 
                          header = TRUE,
                          stringsAsFactors = FALSE) #,
df_data_all <- df_data_all  %>% mutate_all(as.character) %>% 
  filter(!(SUBJID %in% c(96, 104,	122,	167,	178,	219,	268,	279,	303,	308,	534,	701))) # 302 added because of the missing point in LPS
df_data_all$SUBJID <- as.character(df_data_all$SUBJID)
df_data_all <- df_data_all[order(df_data_all$SUBJID),]


for (labkeyTable in c("../LabExMI_ancestry_PCs.csv")){
  ...
  df_data <- read.table(file = labkeyTable,
                        ...
                        header = TRUE,
                        stringsAsFactors = FALSE)#,
                        #check.names=FALSE) # to avoid replacement of dashes by dots
  ...
  curr_col_names_num <- c("PC1","PC2", "PC3","PC4", "PC5","PC6","PC7","PC8","PC9","PC10", "PC11","PC12","PC13","PC14","PC15", "PC16", "PC17","PC18","PC19", "PC20")
  col_names_num <- c("PC1","PC2", "PC3","PC4", "PC5","PC6","PC7","PC8","PC9","PC10", "PC11","PC12","PC13","PC14","PC15", "PC16", "PC17","PC18","PC19", "PC20")
...
#df_data_all <- df_data_all %>% dplyr::select(-c("AGE.V0","AGE","SEX"))
}
...
for (stimul in c("SEB", "E.coli", "BCG", "C.albicans",  "CD3+CD28", "LPS", "PolyIC", "Null","Influenza","IL1b", 'IFNg',"TNFa" )){  
  ...
  ggsave(plot=marrangeGrob(grobs=heatmapList, nrow=4, ncol=3),width = 8.27, height = 11.69, filename = paste0(pathRes, '/CytokinesVSGeneticPCs_', Sys.Date(), '.pdf')) # for A4 format

  
   write.table(as.data.frame(melted_matDATA),file=paste0(pathRes, "/AdjPvalGeneticPCsx10_", stimul,"_",Sys.Date(),".txt"), quote=F, row.names = F, col.names = T, sep="\t")
  }
#}
