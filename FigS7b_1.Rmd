...
for (crit in setdiff(colnames(cglist), "SUBJID")){
  ...
  # matDATAselect2 <- t(matDATAselect)
  # matDATAselect2 <- matDATAselect2[,colSums(matDATAselect2) >= 1]
  # rowM <- colnames(matDATAselect2)
  # 
  significatif[[stimul]]  <- colM
  # significatifProt[[stimul]]  <- rowM
  ...
  #signiProt <- lapply(matList, function(mm) apply(mm,2,function(x) names(which(x==1))))
